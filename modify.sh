...
# 4 following functions responsible for finding and dealing with handing over a task of changing name of a particular file if it was found (for cases of changing names with or without recursion and using or not using sed) - path is present in $1, sed pattern in $2
...
if test "${input[0]+isset}"   #if it exists take the path given as an argument
then
ChangeNamesRec ${input[0]};
else
echo "Improper path";
exit;
fi
...
if [[ ${#input[*]} == 1 ]]   
then 
if test "${input[1]+isset}"   #checking the sed
then
